...
fn apply_template<'d>(conf: &'d Yaml, node: &'d Option<Node>, tpl: &'d str) -> String {
    ...
    match *node {
        Some(ref node_conf) => {
            ...
            vars.push(Var::from("node_envs", node_conf.conf["envs"].as_str().unwrap_or("")));
            ...
            vars.push(Var::from("node_pg_port", &node_conf.conf["pgPort"].as_i64().unwrap_or(30303).to_string()));
        }
        None => {}
    }
